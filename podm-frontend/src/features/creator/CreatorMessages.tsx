...
const ConversationListItem = ({ conversation, isActive, onClick }: { conversation: ConversationWithFan; isActive: boolean; onClick: () => void; }) => (
    <div onClick={onClick} className={`flex items-center p-3 rounded-lg cursor-pointer transition-colors duration-200 ${isActive ? 'bg-purple-900/50' : 'hover:bg-gray-700/50'}`}><div className="relative mr-3"><img className="w-12 h-12 rounded-full" src={conversation.fan.profile.avatar} alt={conversation.fan.profile.name} />{conversation.lastMessage && !conversation.lastMessage.isRead && <span className="absolute top-0 right-0 block h-3 w-3 rounded-full bg-pink-500 border-2 border-gray-800"></span>}</div><div className="flex-1 min-w-0"><div className="flex justify-between items-center"><p className={`font-bold text-sm ${isActive ? 'text-purple-200' : 'text-gray-200'}`}>{conversation.fan.profile.name}</p><p className="text-xs text-gray-500">{new Date(conversation.updatedAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p></div><p className="text-sm text-gray-400 truncate">{conversation.lastMessage?.text || 'No messages yet'}</p><div className="flex items-center mt-1 text-xs text-green-400 font-semibold"><DollarSign className="w-3 h-3 mr-1" />{(conversation.fan.totalSpent || 0).toFixed(2)}</div></div></div>
);
const MessageBubble = ({ message, isMe }: { message: Message; isMe: boolean; }) => {
    ...
    return (<div className={`flex ${justifyClass}`}><div className={`max-w-xs lg:max-w-md p-1 rounded-2xl ${bubbleClass}`}>{message.text && <p className="px-3 py-2">{message.text}</p>}{message.content && (<div className="space-y-2 bg-black/20 rounded-xl p-2"><img src={message.content.thumbnailUrl} alt="Content thumbnail" className="rounded-lg"/><div className="text-center text-xs p-1 font-bold text-pink-300 bg-pink-500/20 rounded-lg">PPV Content - ${(message.content.price / 100).toFixed(2)}</div></div>)}</div></div>);
};
...
const CreatorMessagesPage = () => {
    ...
    // Effect for handling WebSocket connection and listeners
    useEffect(() => {
        socket.connect();
        ...
        socket.on('new_message', (newMessage: Message) => {
            setMessages(prev => {
                // If the message is already in our state, don't add it again.
    ...
    // Effect for fetching messages and joining/leaving rooms
    useEffect(() => {
    ...
    const handleSendMessage = async (e: React.FormEvent) => {
        ...
        setNewMessageText(''); // Clear UI immediately
        try {
            // Send the message but DO NOT update state here.
            // The socket listener is now the single source of truth.
            await apiClient.sendMessage(activeConversation.fan._id, textToSend);
        } catch (error) {
