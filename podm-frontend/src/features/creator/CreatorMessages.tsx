...
interface ConversationWithFan {
    _id: string | null; fan: { _id: string; profile: { name: string; avatar: string; }; totalSpent: number; isNewSubscriber: boolean; }; lastMessage?: { text?: string; isRead: boolean; }; updatedAt: string;
}
...
// --- Reusable Sub-Components (Remain the same) ---
const ConversationListItem = ({ conversation, isActive, onClick }: { conversation: ConversationWithFan; isActive: boolean; onClick: () => void; }) => (
    <div onClick={onClick} className={`flex items-center p-3 rounded-lg cursor-pointer transition-colors duration-200 ${isActive ? 'bg-purple-900/50' : 'hover:bg-gray-700/50'}`}>
        <div className="relative mr-3"><img className="w-12 h-12 rounded-full" src={conversation.fan.profile.avatar} alt={conversation.fan.profile.name} />{conversation.lastMessage && !conversation.lastMessage.isRead && <span className="absolute top-0 right-0 block h-3 w-3 rounded-full bg-pink-500 border-2 border-gray-800"></span>}</div>
        <div className="flex-1 min-w-0"><div className="flex justify-between items-center"><p className={`font-bold text-sm ${isActive ? 'text-purple-200' : 'text-gray-200'}`}>{conversation.fan.profile.name}</p><p className="text-xs text-gray-500">{new Date(conversation.updatedAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p></div><p className="text-sm text-gray-400 truncate">{conversation.lastMessage?.text || 'No messages yet'}</p><div className="flex items-center mt-1 text-xs text-green-400 font-semibold"><DollarSign className="w-3 h-3 mr-1" />{(conversation.fan.totalSpent || 0).toFixed(2)}</div></div>
    </div>
);
...
const MessageBubble = ({ message, isMe }: { message: Message; isMe: boolean; }) => {
    ...
    // Use Primary Purple for the creator (me) and neutral gray for the fan (them)
    const bubbleClass = isMe
        ? 'bg-pink-600 text-white rounded-bl-none' // Creator is now purple
    ...
    return (
        <div className={`flex ${justifyClass}`}>
            <div className={`max-w-xs lg:max-w-md p-1 rounded-2xl ${bubbleClass}`}>
                ...
                {message.content && (
                    <div className="space-y-2 bg-black/20 rounded-xl p-2"><img src={message.content.thumbnailUrl} alt="Content thumbnail" className="rounded-lg"/><div className="text-center text-xs p-1 font-bold text-pink-300 bg-pink-500/20 rounded-lg">PPV Content - ${(message.content.price / 100).toFixed(2)}</div></div>
                )}
            </div>
        </div>
    );
...
const CreatorMessagesPage = () => {
    ...
    useEffect(() => {
        ...
        const channel = supabase.channel(`messages:${activeConversation._id}`).on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'messages', filter: `conversation_id=eq.${activeConversation._id}` }, (payload) => {
            ...
            if (newMessage.senderId !== currentCreator?._id) {
                setMessages((prevMessages) => [...prevMessages, newMessage]);
            }
        }).subscribe();
        return () => { supabase.removeChannel(channel); };
    }, [activeConversation, currentCreator]);
    ...
    const handleSendMessage = async (e: React.FormEvent) => {
        ...
        setNewMessageText('');
        try {
            const response = await apiClient.sendMessage(fanId, textToSend);
        ...
        <div className="flex h-screen bg-gray-900 text-gray-200">
            <div className={`w-full md:w-1/3 lg:w-1/4 bg-gray-800 border-r border-gray-700 flex flex-col ${activeConversation && 'hidden md:flex'}`}>
                ...
                <div className="flex-1 overflow-y-auto p-2 space-y-1">
                    {isLoadingConvos ? <p className="p-4 text-center">Loading subscribers...</p> : conversations.map(convo => <ConversationListItem key={convo.fan._id} conversation={convo} isActive={selectedFanId === convo.fan._id} onClick={() => setSelectedFanId(convo.fan._id)} />)}
                </div>
            </div>
                    ...
                    <>
                        ...
                        <div className="flex-1 p-4 space-y-4 overflow-y-auto">
                            {isLoadingMessages ? <p className="text-center">Loading messages...</p> : messages.map((msg) =>
                                // --- REFACTORED COMPONENT CALL ---
                                <MessageBubble
                                    key={msg._id}
                                    message={msg}
                                    isMe={msg.senderId === currentCreator?._id}
                                />
                            )}
                        </div>
                        <form onSubmit={handleSendMessage} className="p-4 bg-gray-800 border-t border-gray-700"><div className="flex items-center bg-gray-700 rounded-full p-1"><Button type="button" variant="ghost" size="sm" className="p-2 h-auto"><Paperclip className="w-6 h-6" /></Button><input type="text" value={newMessageText} onChange={e => setNewMessageText(e.target.value)} placeholder="Type a message..." className="flex-1 bg-transparent px-3 outline-none" /><Button type="button" variant="ghost" size="sm" className="p-2 h-auto"><Lock className="w-6 h-6" /></Button><Button type="submit" size="sm" className="p-2 h-auto rounded-full ml-2"><Send className="w-5 h-5" /></Button></div></form>
                    </>
                ) : (
