...
- Renewal crash-recovery (H-04) is **mitigated**: `claim_subscription_renewal` (from `add_renewal_claim_pending_guard.sql`, applied live) requires `renewal_pending_tx_hash IS NULL`, so a subscription whose worker broadcast is only resolvable by the reconciliation phase — never re-claimed/re-broadcast. `server/jobs/renewSubscriptions.ts` captures `tx.hash` **immediately** after `sendTransaction` (before `tx.wait()`), durably stores it (`markRenewalPending`), and runs phase 1 `reconcilePendingRenewals()` before processing new due renewals: stored hash → receipt status 1 + verified `SubscriptionRenewed` log → `finalizeSuccessfulRenewal` (idempotent via `findTransactionByBlockchainTxHash`, writes `payment_method='crypto'`/`payment_currency='USDC'`/`chain_id`); status 0 → `clearRenewalPending` (safe retry); no receipt → deferred; past `RENEWAL_NO_RECEIPT_RELEASE_MS` (default 1h) → cleared (tx presumed dropped). Never re-broadcasts. Verified against the live DB with a local Hardhat `FakeRenewal` fixture (emits the exact `SubscriptionRenewed` event, keeper-guarded, `renewCount` + `setRevertNext`): state-machine harness **21/21 PASS** (claim contention, crash-recovery, RPC-timeout no-double-charge, never-mined release) and 9 Jest unit tests. Test-only fixture `contracts/contracts/FakeRenewal.sol` is never deployed to a real network.
