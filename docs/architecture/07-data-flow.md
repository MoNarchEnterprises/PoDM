...
**File:** `docs/architecture/07-data-flow.md`
**Status:** Complete
...
Inline markers annotate data that requires special handling:

| Marker | Category | Examples |
|---|---|---|
| `[PII]` | Personally Identifiable Information | Email, name, username, bio, avatar, wallet address, messages |
| `[SECRET]` | Secrets (must never reach client) | API keys, service role keys, JWT secret, R2 credentials |
| `[AUTH]` | Authentication data | JWT tokens, password hashes, impersonation headers |
| `[PAYMENT]` | Payment data | Transaction amounts, fees, wallet addresses, blockchain hashes |
| `[AI_PROMPT]` | AI Prompt | Image/video content sent to AI API for caption generation |
| `[AI_RESPONSE]` | AI Response | Generated caption text stored as content description |
...
**Login/Signup forms** in `podm-frontend/src/features/auth/AuthModal.tsx:30-34` — user submits `{ email, password }` (plus `username`, `role` for signup). `[PII]` `[AUTH]` Data is sent as POST body to `/api/v1/auth/login` or `/api/v1/auth/signup`.

Supabase **password reset email link** calls `authSupabase.auth.resetPasswordForEmail()` (`auth.service.ts:289`). `[PII]`

### Validation

- **Controller-level** (`auth.controller.ts:40-42`): Manual `if (!email || !password)` checks. `[AUTH]` No schema validation library (Joi/Zod) on auth routes.
- **Supabase Auth API**: Validates email format, password strength (Supabase-managed), checks credentials against stored hash. `[AUTH]`
- **Auth middleware** (`auth.middleware.ts:55`): `supabase.auth.getUser(token)` validates JWT signature + expiry remotely via Supabase Auth API. `[AUTH]` No local JWT decode.
- **Password change** (`auth.service.ts:259`): Verifies current password via `authSupabase.auth.signInWithPassword()` before allowing update. `[AUTH]`
...
- **`[SECRET]`**: `SUPABASE_SERVICE_ROLE_KEY` and `JWT_SECRET` committed in `podm-frontend/.env` (exposed to client).
- **`[PII]`**: Email, username, full name, avatar URL.
- **`[AUTH]`**: JWT stored in `localStorage` (XSS-vulnerable). `impersonating_user_id` in plaintext storage.
- **`[PII]` `[AUTH]`**: Auth debug log (`auth.middleware.ts:20-23`) appends user IDs/emails to `server/debug.log` — PII liability if not rotated.
...
**Registration form** (`AuthModal.tsx:30-34`) — username, email, password, role (fan/creator). `[PII]` **Profile edit forms** in `podm-frontend/src/features/settings/` — bio, display name, avatar, social links, preferences. `[PII]` **Creator settings** in `CreatorSettings.tsx` — subscription tiers, welcome message, payout preferences. `[PII]` `[PAYMENT]`
...
- **`[PII]` `[PAYMENT]`**: `stripe_customer_id`, `stripe_account_id` columns exist but are not actively populated.
- **`[PII]` `[PAYMENT]`**: Creator earnings exposure — Admin routes expose `total_earnings` and `follower_count`.
- **`[PII]`**: Verification docs — 60-second signed URLs returned to admin; contain sensitive ID/selfie data.
...
- **`[PII]`**: Creator media stored in R2 private bucket — access requires signed URLs.
- **`[PII]`**: Fan usernames in watermarks — `@<fan.username>` composited into watermarked images.
...
- **`[PII]` `[PAYMENT]`**: Fan and creator IDs linked in subscription records.
- **`[PAYMENT]`**: Column repurposing — `stripe_subscription_id` stores `txHash`; semantic misuse could cause confusion.
...
**Wallet transaction**: Fan sends USDC via `PoDMPaymentProtocol.sol` smart contract using their Web3 wallet (MetaMask, WalletConnect, embedded). `[PAYMENT]` Contract emits event with `totalAmount`, `platformFee`, `creatorAmount`, fan/creator addresses.

**Client submission**: Frontend captures `txHash` from wallet after confirmation and sends to backend via `POST /api/v1/payments/crypto/verify` with `{ txHash, creatorId, amountInCents, transactionType, relatedId? }`. `[PAYMENT]`

**Subscription flow**: `POST /api/v1/subscriptions` calls `verifyAndRecordBasePayment()` internally. `[PAYMENT]`
...
- **`[PII]` `[PAYMENT]`**: Crypto wallet addresses stored in plaintext in `profiles`.
- **`[PAYMENT]`**: Transaction hashes (public ledger data) stored in DB.
- **`[PAYMENT]`**: Payment amounts (cents) accessible to fan (own), creator (own), admin (all).
- **`[PAYMENT]`**: No refund/dispute mechanism — crypto is final.
- **`[PAYMENT]`**: Sandbox bypass — Any `0x0000`-prefixed hash skips verification; **critical vulnerability** allowing fake transactions.
- **`[SECRET]`**: Platform fee logic (12.5%) hardcoded — business-sensitive.
...
- **`[PAYMENT]`**: Payout amounts — Creator earnings data; accessible to creator and admin.
- **`[PAYMENT]`**: Mocked off-ramp — No real money movement; earnings sit in internal ledger only.
- **`[PAYMENT]`**: Balance race condition — Available balance read at request time, not locked; concurrent payout requests could over-draft.

---

## 7.5. Stripe Setup Intent & Payment Method Management

**Status:** ABORTED. Stripe Setup Intent (`createSetupIntent`, `updateMyPaymentMethod`/`updateFanPaymentMethod`, `POST /users/me/setup-payment-method`, `PUT /users/me/payment-method`) was fully removed from the codebase. No backend routes, controllers, or service methods exist. All payment method management is now handled via crypto wallet address (`profiles.crypto_wallet_address`) through a different mechanism. Do not re-introduce without a new Payment Method architecture decision.
...
- **`[PII]`**: Message text stored in plaintext in DB. No E2EE.
- **`[PII]` `[PAYMENT]`**: User names, avatar URLs, `totalSpent` (exposed to creators).
- **`[PII]`**: Voice messages — 7-day signed URLs; extended window for potential URL leakage.
...
**Manual trigger**: Creator clicks "AI Caption" button on `DraftCard.tsx`. `BulkUploadPage.tsx:handleGenerateCaption(draft.id, draft.file)` calls `apiClient.generateCaption(image)` → `POST /api/v1/ai/caption`. `[AI_PROMPT]`

**Batch trigger**: "Caption All" button iterates all drafts without captions, calls `handleGenerateCaption()` sequentially with 5-second delay (30-second on 429). `[AI_PROMPT]`
...
- **Prompt construction** (`ai.service.ts:39`): Hardcoded string: "Write ONE witty, enticing caption for this image or video in English only... Max 20 words." `[AI_PROMPT]`
- **Image encoding**: File buffer → base64 data URI (`data:image/jpeg;base64,...`). `[AI_PROMPT]` — full user media encoded for transmission.
- **Model selection** (`ai.service.ts:30`): `process.env.AI_MODEL_ID || "google/gemma-3-27b-it:free"`.
- **API routing** (`ai.service.ts:7-16`): If API key starts with `sk-or-v1`, target OpenRouter; else target OpenAI. `[SECRET]`
- **SDK invocation**: `openai.chat.completions.create()` with `max_tokens: 100`.
- **Response parsing** (`ai.service.ts:61`): `response.choices[0]?.message?.content || "Just posted! ✨ #newcontent"` — no parsing or sanitization. `[AI_RESPONSE]`
- **Fallback** (no API key): Returns `"Enjoying the moment! ✨ #vibes (AI Key Missing)"`. `[AI_RESPONSE]`
- **Title truncation**: First 50 characters of caption become content `title`. `[AI_RESPONSE]`
...
- **`[SECRET]`**: `AI_API_KEY` — `process.env.AI_API_KEY` critical secret. Key prefix determines provider (OpenAI or OpenRouter).
- **`[AI_PROMPT]`**: User media — full-resolution images/videos transmitted to third-party AI API without user consent dialog.
- **`[AI_PROMPT]`**: No NSFW moderation before transmission — potential ToS violation with AI providers.
- **`[AI_RESPONSE]`**: No audit trail — AI-generated content indistinguishable from manual input.
...
- **`[PII]` `[PAYMENT]`**: Full user profiles (names, emails, earnings, follower counts) exposed to admins.
- **`[PII]`**: Verification docs — Government ID and selfie images via temporary signed URLs.
- **`[PAYMENT]` `[SECRET]`**: Financial data — Platform revenue, per-creator earnings, commission rate configuration.
...
- **`[PII]`**: Username embedded in referral code — anyone seeing the code knows the referrer's username.
- **`[SECRET]`**: Hardcoded amounts — $50 cash, $25 speed bonus, $750 milestone threshold; business-sensitive incentive structure.
...
- **`[PAYMENT]`**: No actual payout mechanism — Bonuses tracked in DB but no payment integration to disburse.
...
- **`[SECRET]`**: Capacity cap — Business-sensitive; max 50 Enclave spots.
- **`[PII]`**: Referral linkage — Application linked to referrer via `referral_code`; PII cross-reference.
...
- **`[PAYMENT]`**: Payment data in weighted draw — `transactions` table queried for real payment amounts (cents) to compute entry weights.
...
- **`[PII]`**: Full conversation history — User messages with sender names stored in JSONB array.
...
### Secrets (must never reach client) `[SECRET]`
...
### AI Data `[AI_PROMPT]` `[AI_RESPONSE]`

| Data | Storage | Sensitivity |
|---|---|---|
| `AI_API_KEY` | `server/.env` | `[SECRET]` Critical — API access |
| `AI_MODEL_ID` | `server/.env` | Low — model selection |
| User media (image/video) | Transmitted to AI API | `[AI_PROMPT]` High — creator content sent to third-party |
| AI prompts | Hardcoded in `ai.service.ts:39` | `[AI_PROMPT]` Low — static text |
| AI responses | `content.description` | `[AI_RESPONSE]` Medium — published content |
